<script lang="ts">
	...
	const celebrate = (
		newTrack: Track,
		newPlayer: Player,
		newIsOtherPlayer: boolean,
		newHasWon: boolean,
		newIsfinalWin: boolean
	) => {
		track = newTrack;
	...
	const celebrateTrack = async (
		track: Track,
		player: Player,
		isOtherPlayer: boolean,
		hasWon: boolean,
		isFinalWin: boolean,
		millisecondsToSleep?: number
	) => {
		isCelebrating = true;
	...
	const celebrateRound = async (
		...
		const player =
			winner ?? findPlayerInGameSessionUpdate(update, update.rounds[roundIndex].playerId);
	...
	const processCelebrations = async (
		currentGame: GameSessionUpdate | null,
		update: GameSessionUpdate | null,
		newMe: Player | null
	) => {
		if (!update) return;
	...
	export const update = async ({ oldUpdate, newUpdate, oldMe, newMe }: CelebrationUpdate) => {
		...
		if (!oldUpdate && 'rounds' in newUpdate && newUpdate.rounds.length > 0)
			await celebrateStart(newMe);
...
<div class="celebration" class:hidden={!isCelebrating}>
	...
	<p>
		{isOtherPlayer ? currentPlayer?.name + ' has' : 'You have'}
		{hasWon ? '' : 'not'} won a new track!
	</p>
	{#if track}
		<div class="track">
	...
	{#if isfinalWin}
		<h1>Game Winner!</h1>
		<p>
			Congratulations! {isOtherPlayer ? currentPlayer?.name + ' has' : 'You have'} won the game!
		</p>
	{/if}
</div>
